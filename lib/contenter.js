...
class Contenter {
   ...
   * @returns {Promise<ContenterResult>}
   */
  async get(url = "", options = {}, noCache = false) {
    url = this.normalizeUrl(url);

    if (!url) {
      return new ContenterResult(400);
    }
    ...
    try {
      ...
      if (content) {
        return new ContenterResult(200, content);
      }
  ...
  async forget(url) {
    ...
    return new ContenterResult(202);
  }
